...
# Ventium M9.5 — SoC LES/LDS/LSS/LFS/LGS gate, FULL PER-RECORD DIFFERENTIAL vs qemu-system.
#
...
# firmware (psoclseg.bin) builds a far pointer {off,sel} in RAM and loads it with each of
# LES/LDS/LSS/LFS/LGS, then isa-debug-exits. Each load is synchronous + deterministic, so the
# gen_trace.py --system single-step golden is a valid per-record oracle — the compare checks
# the GPR (offset) + the loaded segment selector at each step, diffing both vs qemu.
#
...
TDIR="$REPO/verif/sys/tests/psoclseg"
...
IMG="$TDIR/psoclseg.bin"                          # the 64 KiB real-mode -bios firmware
GOLD_REF="$TDIR/psoclseg.sys.vtrace.golden"        # committed reference (evidence)
GOLD_GEN="$OUTDIR/psoclseg.sys.vtrace.golden"      # freshly regenerated this run
RTL_OUT="$OUTDIR/psoclseg.rtl.soc.vtrace"
PORT="${PORT:-51271}"
...
say "0. build psoclseg.bin (-bios real-mode LES/LDS/LSS/LFS/LGS firmware)"
...
say "2. confirm psoclseg reaches isa-debug-exit (code 133) under qemu-system-i386"
...
LAST_SITE='"pc":"0x0000002c"'   # the final LDS
grep -q "$LAST_SITE" "$GOLD_GEN" || { echo "FATAL: golden never reaches the final LDS (pc=0x2c)"; exit 1; }
echo "non-vacuous (golden): reaches the final LDS site pc=0x0000002c: OK"

# --- 4. run ventium_soc on the firmware ------------------------------------------
say "4. run ventium_soc on psoclseg.bin"
"$SOC_TB" --image "$IMG" --out "$RTL_OUT" --max-insn "$MAXI" --max-cycles 200000 --quiesce 300
echo "RTL soc trace: $RTL_OUT ($(wc -l < "$RTL_OUT") lines)"
grep -q "$LAST_SITE" "$RTL_OUT" || { echo "FATAL: RTL trace never reaches the final LDS (pc=0x2c) -- a seg-load HALTed"; exit 1; }
echo "non-vacuous (RTL): ventium_soc reaches the final LDS site pc=0x0000002c: OK"
...
echo
if [[ "$CMP" == "0" ]]; then
  echo "SOC-LSEG-GATE-OK  (PER-RECORD DIFFERENTIAL EQUIVALENT)"
  echo "  ventium_soc executes LES/LDS/LSS/LFS/LGS (far-pointer load: GPR<-off, seg<-sel)"
  echo "  byte-identical to qemu-system-i386 over all $(($(wc -l < "$RTL_OUT")-1)) retired instructions."
  echo
  echo "M9.5 SoC LES/LDS family GATE: EQUIVALENT (per-record, full differential)"
else
  echo "M9.5 SoC LES/LDS family GATE: FAIL (compare.py exit $CMP)"
  exit 1
fi
