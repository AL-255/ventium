...
# Ventium M9.5 — SoC SGDT/SIDT (store GDTR/IDTR) gate, FULL PER-RECORD DIFFERENTIAL vs qemu-system.
#
...
# firmware (psocsgdt.bin) stays in 16-bit real mode and executes CPUID for four leaves
...
TDIR="$REPO/verif/sys/tests/psocsgdt"
...
IMG="$TDIR/psocsgdt.bin"                          # the 64 KiB real-mode -bios firmware
GOLD_REF="$TDIR/psocsgdt.sys.vtrace.golden"        # committed reference (evidence)
GOLD_GEN="$OUTDIR/psocsgdt.sys.vtrace.golden"      # freshly regenerated this run
RTL_OUT="$OUTDIR/psocsgdt.rtl.soc.vtrace"
PORT="${PORT:-51272}"
...
say "0. build psocsgdt.bin (-bios real-mode SGDT/SIDT firmware)"
...
say "2. confirm psocsgdt reaches isa-debug-exit (code 133) under qemu-system-i386"
...
LAST_SITE='"pc":"0x00000058"'   # the final readback (idt base31:16 -> bp)
grep -q "$LAST_SITE" "$GOLD_GEN" || { echo "FATAL: golden never reaches the final readback (pc=0x58)"; exit 1; }
echo "non-vacuous (golden): reaches the final readback pc=0x00000058: OK"

# --- 4. run ventium_soc on the firmware ------------------------------------------
say "4. run ventium_soc on psocsgdt.bin"
"$SOC_TB" --image "$IMG" --out "$RTL_OUT" --max-insn "$MAXI" --max-cycles 200000 --quiesce 300
echo "RTL soc trace: $RTL_OUT ($(wc -l < "$RTL_OUT") lines)"
grep -q "$LAST_SITE" "$RTL_OUT" || { echo "FATAL: RTL trace never reaches the final readback (pc=0x58) -- SGDT/SIDT HALTed"; exit 1; }
echo "non-vacuous (RTL): ventium_soc reaches the final readback pc=0x00000058: OK"
...
echo
if [[ "$CMP" == "0" ]]; then
  echo "SOC-SGDT-GATE-OK  (PER-RECORD DIFFERENTIAL EQUIVALENT)"
  echo "  ventium_soc stores GDTR/IDTR via SGDT/SIDT byte-identical to qemu-system-i386"
  echo "  over all $(($(wc -l < "$RTL_OUT")-1)) retired instructions (verified by reading the stored bytes back)."
  echo
  echo "M9.5 SoC SGDT/SIDT GATE: EQUIVALENT (per-record, full differential)"
else
  echo "M9.5 SoC SGDT/SIDT GATE: FAIL (compare.py exit $CMP)"
  exit 1
fi
