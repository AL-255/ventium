...
# Ventium F3 — real-mode IVT INT n / IRET delivery gate, FULL PER-RECORD DIFFERENTIAL vs qemu-system.
#
...
# firmware (psocrmint.bin) stays in 16-bit real mode and executes CPUID for four leaves
...
TDIR="$REPO/verif/sys/tests/psocrmint"
...
IMG="$TDIR/psocrmint.bin"                          # the 64 KiB real-mode -bios firmware
GOLD_REF="$TDIR/psocrmint.sys.vtrace.golden"        # committed reference (evidence)
GOLD_GEN="$OUTDIR/psocrmint.sys.vtrace.golden"      # freshly regenerated this run
RTL_OUT="$OUTDIR/psocrmint.rtl.soc.vtrace"
PORT="${PORT:-51274}"
...
say "0. build psocrmint.bin (-bios real-mode INT/IRET firmware)"
...
say "2. confirm psocrmint reaches isa-debug-exit (code 133) under qemu-system-i386"
...
HANDLER='"pc":"0x0000002a"'   # the INT 0x40 handler ran
RETURNED='"pc":"0x0000001e"'  # IRET returned to the post-INT marker
grep -q "$HANDLER" "$GOLD_GEN" || { echo "FATAL: golden never enters the handler (pc=0x2a)"; exit 1; }
grep -q "$RETURNED" "$GOLD_GEN" || { echo "FATAL: golden never returns from IRET (pc=0x1e)"; exit 1; }
echo "non-vacuous (golden): enters the handler (0x2a) AND returns via IRET (0x1e): OK"

# --- 4. run ventium_soc on the firmware ------------------------------------------
say "4. run ventium_soc on psocrmint.bin"
"$SOC_TB" --image "$IMG" --out "$RTL_OUT" --max-insn "$MAXI" --max-cycles 200000 --quiesce 300
echo "RTL soc trace: $RTL_OUT ($(wc -l < "$RTL_OUT") lines)"
grep -q "$HANDLER" "$RTL_OUT" || { echo "FATAL: RTL never enters the handler (pc=0x2a) -- INT delivery failed"; exit 1; }
grep -q "$RETURNED" "$RTL_OUT" || { echo "FATAL: RTL never returns from IRET (pc=0x1e) -- IRET failed"; exit 1; }
echo "non-vacuous (RTL): ventium_soc enters the handler (0x2a) AND returns via IRET (0x1e): OK"
...
echo
if [[ "$CMP" == "0" ]]; then
  echo "SOC-RMINT-GATE-OK  (PER-RECORD DIFFERENTIAL EQUIVALENT)"
  echo "  ventium_soc delivers a real-mode INT (push FLAGS:CS:IP, IVT vector) + IRET"
  echo "  byte-identical to qemu-system-i386 over all $(($(wc -l < "$RTL_OUT")-1)) retired instructions."
  echo
  echo "F3 real-mode INT/IRET GATE: EQUIVALENT (per-record, full differential)"
else
  echo "F3 real-mode INT/IRET GATE: FAIL (compare.py exit $CMP)"
  exit 1
fi
