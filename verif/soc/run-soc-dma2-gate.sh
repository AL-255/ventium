...
# Ventium M8.8 — Intel 8237A DMA controller (ctrl 1 (secondary) + page regs) device gate.
#
# FULL PER-RECORD DIFFERENTIAL vs qemu-system-i386, same shape as run-soc-dev-gate:
# the psoc8237b image is entirely SYNCHRONOUS (IN/OUT to 0xC0-0xDF + 0x89/0x8B/0x8F,
...
#
# Never weakens / never fakes a sys-diff. Usage: bash verif/soc/run-soc-dma2-gate.sh
set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
TDIR="$REPO/verif/sys/tests/psoc8237b"
...
IMG="$TDIR/psoc8237b.bin"
GOLD_REF="$TDIR/psoc8237b.sys.vtrace.golden"
GOLD_GEN="$OUTDIR/psoc8237b.sys.vtrace.golden"
RTL_OUT="$OUTDIR/psoc8237b.rtl.soc.vtrace"
PORT="${PORT:-51158}"
MAXI=250
...
say "1. build psoc8237b.bin + the ventium_soc --soc TB"
...
say "2. confirm psoc8237b.bin runs to isa-debug-exit (code 133) under qemu-system-i386"
...
# --- 4. run ventium_soc on psoc8237b.bin -----------------------------------------
say "4. run ventium_soc on psoc8237b.bin (8237 DMA ctrl1 register surface)"
...
echo
if [[ "$CMP" == "0" ]]; then
  echo "SOC-DMA2-GATE-OK  (PER-RECORD DIFFERENTIAL EQUIVALENT)"
  echo "  8237 ctrl1: reset status/mask + ch4 addr/count flip-flop round-trip +"
  ...
  echo
  echo "M8.8 SOC DMA2 GATE: EQUIVALENT (per-record, full differential)"
else
  echo "M8.8 SOC DMA2 GATE: FAIL (compare.py exit $CMP)"
  exit 1
fi
