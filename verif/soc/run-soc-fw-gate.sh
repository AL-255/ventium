...
# Ventium F2 — SoC boot-firmware gate (boot + COM1 console banner + clean exit), FULL PER-RECORD DIFFERENTIAL vs qemu-system.
#
...
# firmware (psocfw.bin) stays in 16-bit real mode and executes CPUID for four leaves
...
TDIR="$REPO/verif/sys/tests/psocfw"
...
IMG="$TDIR/psocfw.bin"                          # the 64 KiB real-mode -bios firmware
GOLD_REF="$TDIR/psocfw.sys.vtrace.golden"        # committed reference (evidence)
GOLD_GEN="$OUTDIR/psocfw.sys.vtrace.golden"      # freshly regenerated this run
RTL_OUT="$OUTDIR/psocfw.rtl.soc.vtrace"
PORT="${PORT:-51273}"
MAXI=600  # the banner print loop is ~6 insns/char x ~48 chars + setup

say(){ echo; echo "=== $* ==="; }

# --- 0. build the CPUID firmware -------------------------------------------------
say "0. build psocfw.bin (-bios F2 boot firmware: COM1 banner)"
...
say "2. confirm psocfw reaches isa-debug-exit (code 133) under qemu-system-i386"
...
LAST_SITE='"pc":"0x0000003c"'   # the exit (banner fully printed)
grep -q "$LAST_SITE" "$GOLD_GEN" || { echo "FATAL: golden never reaches the exit (pc=0x3c) -- banner did not finish"; exit 1; }
echo "non-vacuous (golden): reaches the exit pc=0x0000003c (banner printed): OK"

# --- 4. run ventium_soc on the firmware ------------------------------------------
say "4. run ventium_soc on psocfw.bin"
"$SOC_TB" --image "$IMG" --out "$RTL_OUT" --max-insn "$MAXI" --max-cycles 200000 --quiesce 300
echo "RTL soc trace: $RTL_OUT ($(wc -l < "$RTL_OUT") lines)"
grep -q "$LAST_SITE" "$RTL_OUT" || { echo "FATAL: RTL trace never reaches the exit (pc=0x3c) -- firmware HALTed"; exit 1; }
echo "non-vacuous (RTL): ventium_soc reaches the exit pc=0x0000003c (banner printed): OK"
...
echo
if [[ "$CMP" == "0" ]]; then
  echo "SOC-FW-GATE-OK  (PER-RECORD DIFFERENTIAL EQUIVALENT)"
  echo "  ventium_soc boots, inits COM1, prints the banner + exits byte-identical to"
  echo "  qemu-system-i386 over all $(($(wc -l < "$RTL_OUT")-1)) retired instructions."
  echo
  echo "F2 SoC boot-firmware GATE: EQUIVALENT (per-record, full differential)"
else
  echo "F2 SoC boot-firmware GATE: FAIL (compare.py exit $CMP)"
  exit 1
fi
