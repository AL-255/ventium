...
| M3 | x87 FPU | x87 corpus diff-clean vs QEMU (`make m3` exit 0) | ✅ done (x87 functional core: stack/status/control/tag + 80-bit datapath, data movement + normal-operand arithmetic bit-exact vs QEMU; 14-program x87 corpus + 28 integer = 42/42 PASS. BCD (M10) and the **transcendentals (M11/#11, gated `+VEN_TRANSCENDENTAL`)** are now IMPLEMENTED; FSAVE/FRSTOR/FLDENV, unmasked #MF, and non-default **precision** control (PC≠64-bit) remain DEFERRED and HALT loudly) |
...
| M5B | Pin-level 64-bit bus protocol (needs real-chip bus traces) | structural + local SVA (no differential oracle) | ✅ done STANDALONE (biu_p5, 16 cycle types, 19 mutation-validated SVA + 76 directed checks; commit 3e82269). **M5B-int (2026-06-05): the bus is now wired into `rtl/` behind a default-OFF `bus_mode`.** `biu_p5` moved to its canonical home `rtl/bus/biu_p5.sv` (added to `ventium.f`); `rtl/bus/biu.sv` rewrote the M0 stub into a gated SUBSYSTEM = a FRONT 32b adapter + the `biu_p5` instance (driving the real ADS#/A/BE#/M-IO#/D-C#/W-R#/D63-0/BRDY# pins) + a loopback pin-level responder; `ventium_top.sv` adds the `bus_mode` input + mux. **bus_mode=0 (DEFAULT) = BIT-IDENTICAL:** `make verify` GREEN (57/57 func + every M4 integer + M5 FP/cache cycle band met, e.g. mb_depadd C=3497 / mb_imiss C=24149 / mb_dmiss C=20544 unchanged); the subsystem is wholly inert (`.c_req(bus_mode?core_mem_req:1'b0)` in `ventium_top.sv:345` ⇒ 0 ADS# pulses — VCD-confirmed; back-side bus access counts identical, t_stack 592 reads/48 writes in BOTH modes); pseg+pmode sys gates RTL-SYS-DIFF-OK EQUIVALENT. **bus_mode=1 = FUNCTIONAL + SVA:** the 12-program corpus (smoke,t_mem,t_stack,t_string,t_mul,t_loop,t_callret,t_rep,t_rotate,t_div + x87 tx_addsub,tx_ldst) run through `tb_ventium --bus-mode` is func-EQUIVALENT vs the QEMU golden (12/12, `compare.py --mode func` exit 0); the bus pins genuinely toggle (VCD: t_stack 73 ADS# + 72 BRDY# beats in mode 1 vs 0 in mode 0); the 19 biu_p5 SVA HOLD in-system (the `bind biu_p5 biu_p5_sva` + `--assert` `rtl-sva` build ran the full corpus with ZERO assertion fires). **STANDALONE bus gate still GREEN** (`verif/bus/run.sh`: lint OK + 76 directed checks PASS + 19 SVA, ALL GREEN). Lint clean (`verilator --lint-only -Wall -Wno-UNUSED -f ventium.f`, 0 warn/err; biu_p5 in the build). **HONEST FRAMING (no cycle oracle):** there is NO pin-level cycle/timing oracle (p5model = fixed miss penalty; QEMU has no pin trace), so NO cycle/pin-exact claim is made through the bus. In-system traffic is SINGLE-CYCLE only (KEN#/NA#/HOLD/BOFF#/AHOLD/EADS# tied deasserted — burst/pipeline/locked/snoop/backoff stay validated ONLY by the standalone gate). And per the M5B-int review (med, applied as the honest-doc fix): `biu_p5` here is a PROTOCOL EXERCISER, not a faithful data carrier — because the core gets a COMBINATIONAL ack it advances before biu_p5's registered req_ack, so the loopback returns SOME real back-side word on d_in but generally the core's NEXT access, NOT the word at the address on the pins; this does NOT affect any gated result (the core's data is `c_rdata = m2_rdata` combinational + independent of biu_p5, the SVA check protocol timing not data values, and bus_mode=0 bypasses it). All `rtl/bus/biu.sv` comments corrected to state this plainly. |
...
| M11 | x87 transcendentals (#11) | each engine bit-exact vs its oracle; default build byte-identical (`make verify` 77/77) | ✅ done (all 8 — F2XM1/FYL2X/FPTAN/FPATAN/FYL2XP1/FSINCOS/FSIN/FCOS — as iterative microcoded engines gated `+VEN_TRANSCENDENTAL`. **Group B** (F2XM1/FPATAN/FYL2X/FYL2XP1) = bit-exact vs qemu-i386, a verbatim softfloat transcription validated all 4 RC (`tools/p5xtrans/qref.c` + `qref_validate.py`). **Group A** (FSIN/FCOS/FSINCOS/FPTAN) can't be graded vs qemu (qemu uses host glibc at DOUBLE precision), so = bit-exact vs a shared-polynomial silicon model (octant reduction + Taylor, all floatx80 ops), itself ~1.8 ulp vs `__float128` quad — i.e. MORE faithful than qemu's double. Engines: `rtl/fpu/fpu_{f2xm1,fpatan,fyl2x,fsincos}.sv` + `fpu_trsc_wideint.svh`; new `S_TRSC_BUSY`. Gates: `verif/trsc/run-*-{,core-}gate.sh`. Quake's only transcendental (FPATAN) is in → F4 unblocked. **#6** (FP scoreboard from real engine done) closed as by-design — the cycle model keeps the fixed P5 latencies, see `core.sv:805`) |
...
- **Transcendentals** FSIN/FCOS/FSINCOS/FPTAN/FPATAN/F2XM1/FYL2X/FYL2XP1 — _(M3-era
  deferral; **SUPERSEDED — all 8 IMPLEMENTED in M11/#11** under `+VEN_TRANSCENDENTAL`,
  see the M11 row + `docs/m11-transcendental-spec.md`. They still HALT in the DEFAULT
  build, so `tx_deferred_halt` (which uses FSIN) stays valid.)_
