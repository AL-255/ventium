...
| 6 | Expand AP-500 fast-path coverage | **DONE (batches 1-3) + SPEC'd-deferred (batches 4-5)** | Batches 1-2 (accumulator-imm32 ALU `05/0D/15/1D/25/2D/35/3D`; reg-form `81 /r` + `C7 /0` imm32; `D1` shift-by-1) now fast-pathed in `rtl/core/decode.sv` → they PAIR where they serialized; func byte-identical, gated by NEW `mb_accimm`+`mb_rmimm`+`mb_sh1` PAIR bands (pairing 50% / abs-cyc +0.35%). Batches 4-5 (byte forms `04`/`A8`, PUSH/POP, near branches, memory) ordered by risk in `docs/fastpath-coverage-spec.md` §3. |
