...
| 2 | x87 strong for covered operands, not complete P54C x87 (NaN/inf/denormal/non-default PC, transcendentals, BCD FP, env save/restore, unmasked exceptions deferred) | **DONE (boundary tests) + transcendentals + BCD DONE; rest SPEC'd-deferred** | x87 boundary tests `verif/tests/tx_*` make the coverage boundary machine-checkable. **All 8 transcendentals now IMPLEMENTED (#11, gated `+VEN_TRANSCENDENTAL`): F2XM1/FPATAN/FYL2X/FYL2XP1 bit-exact vs qemu-i386, FSIN/FCOS/FSINCOS/FPTAN bit-exact vs a shared-poly silicon model (~1.8 ulp); `verif/trsc/`, `docs/m11-transcendental-spec.md`.** BCD FBLD/FBSTP DONE (M10). NaN/inf/denormal/non-default-PC and env save/restore completion remains deferred per `docs/m3-fpu-spec.md` §"deferred". |
