...
| 4 | Iterative divider (P5 occupancy, EDX:EAX coupling, `#DE`) | **DONE (occupancy + EDX:EAX latency) + SPEC'd-deferred (`#DE`)** | DIV/IDIV now charge the p5model occupancy (17/25/41 DIV, 22/30/46 IDIV) via a deferred penalty in `rtl/core/core_exec.svh`; gated by NEW bands `mb_div8/16/32`+`mb_idiv32` in `verif/m5_metrics.py` (all PASS, abs-cyc within 10%). `#DE` (div-by-0 / quotient-overflow) is the next increment — spec in `docs/m5-div-spec.md` §3.3. |
| 5 | Staged multiply timing (~10cy, U-pipe, non-pairable) | **SPEC'd-deferred** | `docs/m5-mul-spec.md` (occ≈10, NP) |
| 6 | Expand AP-500 fast-path coverage | **SPEC'd-deferred** | `docs/fastpath-coverage-spec.md` (gap table + first batch: accumulator-imm, r/m-imm, push/pop, near branches) |
