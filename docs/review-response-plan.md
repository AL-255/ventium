...
| 6 | Expand AP-500 fast-path coverage | **DONE (batch 1) + SPEC'd-deferred (batches 2-5)** | Batch 1 (accumulator-imm32 ALU `05/0D/15/1D/25/2D/35/3D`) now fast-pathed in `rtl/core/decode.sv` → they PAIR where they serialized; func byte-identical, gated by NEW `mb_accimm` PAIR band (pairing 50% / abs-cyc +0.35%). Batches 2-5 (byte forms, `81`/`C7`, PUSH/POP, near branches, memory) ordered by risk in `docs/fastpath-coverage-spec.md` §3. |
